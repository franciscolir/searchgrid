...
export default function Dashboard({ id }: { id?: string }) {
  ...
  async function createMission() {
    ...
    const body: any = { title, description: desc, polygon: drawPoints };
    ...
    if (res.ok) {
      ...
      setTitle(''); setDesc(''); setDrawPoints([]); setCreating(false); setUseKeyword(false);
      setMissionId(m.id);
    }
  }
      ...
      <div class="container">
        ...
        <div class="card">
          ...
          <button class="btn btn-primary" onClick={() => { setCreating(true); setDrawMode(true); }} disabled={drawMode}>
            {drawMode ? 'Dibujando poligono...' : 'Dibujar area de busqueda'}
          </button>
          {drawPoints.length > 0 && (
            <div class="draw-info">
              <p>{drawPoints.length} puntos marcados. <button class="btn btn-sm" onClick={() => setDrawPoints([])}>Limpiar</button></p>
              <p class="hint">Haz clic en el mapa para marcar vertices del poligono</p>
              <button class="btn btn-primary" onClick={createMission}>Crear Mision</button>
            </div>
          )}
        </div>
        {creating && (
          <div style={{ height: '400px', marginTop: '1rem' }}>
            <MapView drawMode={drawMode} drawPoints={drawPoints} onDrawPoint={(pt) => setDrawPoints(prev => [...prev, pt])} center={drawPoints.length > 0 ? drawPoints[0] : undefined} />
          </div>
        )}
      </div>
    );
  ...
  return (
    <div class="container">
      ...
      <div style={{ height: '60vh', border: '2px solid #1e3a5f', borderRadius: '8px', overflow: 'hidden' }}>
        <MapView polygon={mission?.polygon} sectors={sectors} searchers={searchers.filter(s => s.lat)} />
      </div>
    </div>
  );
}